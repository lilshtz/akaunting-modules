        ...
        <x-form.container>
            <form id="journal-form" method="POST" action="{{ route('double-entry.journals.store') }}">
                ...
                <x-form.section>
                    ...
                    <x-slot name="body">
                        <x-form.group.date name="date" label="{{ trans('general.date') }}" value="{{ old('date', date('Y-m-d')) }}" />

                        <x-form.group.text name="reference" label="{{ trans('double-entry::general.reference') }}" value="{{ old('reference') }}" not-required />

                        <x-form.group.textarea name="description" label="{{ trans('general.description') }}" value="{{ old('description') }}" not-required />

                        <x-form.group.select name="basis" label="{{ trans('double-entry::general.basis') }}" :options="['accrual' => trans('double-entry::general.bases.accrual'), 'cash' => trans('double-entry::general.bases.cash')]" :selected="old('basis', 'accrual')" />

                        <x-form.group.select name="status" label="{{ trans('general.status') }}" :options="['posted' => trans('double-entry::general.statuses.posted'), 'draft' => trans('double-entry::general.statuses.draft')]" :selected="old('status', 'posted')" />
                        ...
                        <x-form.group.date name="next_recurring_date" label="{{ trans('double-entry::general.next_recurring_date') }}" value="{{ old('next_recurring_date') }}" not-required />
                    </x-slot>
                </x-form.section>
                            ...
                            <table class="w-full mb-4">
                                ...
                                <tbody id="lines-body">
                                    <tr class="line-row border-b">
                                        <td class="px-2 py-2">
                                            <select name="lines[0][account_id]" class="w-full border rounded px-2 py-1.5 text-sm" required>
                                                <option value="">--</option>
                                        ...
                                        <td class="px-2 py-2"><input type="number" name="lines[0][debit]" step="0.01" min="0" value="0" class="w-full border rounded px-2 py-1.5 text-sm text-right debit-input" onchange="updateTotals()"></td>
                                        <td class="px-2 py-2"><input type="number" name="lines[0][credit]" step="0.01" min="0" value="0" class="w-full border rounded px-2 py-1.5 text-sm text-right credit-input" onchange="updateTotals()"></td>
                                        <td class="px-2 py-2"><input type="text" name="lines[0][description]" class="w-full border rounded px-2 py-1.5 text-sm"></td>
                                        <td class="px-2 py-2"></td>
                                    </tr>
                                    <tr class="line-row border-b">
                                        <td class="px-2 py-2">
                                            <select name="lines[1][account_id]" class="w-full border rounded px-2 py-1.5 text-sm" required>
                                                <option value="">--</option>
                                                @foreach ($accounts as $id => $name)
                                                    <option value="{{ $id }}">{{ $name }}</option>
                                                @endforeach
                                        ...
                                        <td class="px-2 py-2"><input type="number" name="lines[1][debit]" step="0.01" min="0" value="0" class="w-full border rounded px-2 py-1.5 text-sm text-right debit-input" onchange="updateTotals()"></td>
                                        <td class="px-2 py-2"><input type="number" name="lines[1][credit]" step="0.01" min="0" value="0" class="w-full border rounded px-2 py-1.5 text-sm text-right credit-input" onchange="updateTotals()"></td>
                                        <td class="px-2 py-2"><input type="text" name="lines[1][description]" class="w-full border rounded px-2 py-1.5 text-sm"></td>
                                        <td class="px-2 py-2"></td>
                                    </tr>
                                </tbody>
                                <tfoot>
