<x-layouts.admin>
    <x-slot name="title">{{ trans('general.title.new', ['type' => trans('double-entry::general.journal_entry')]) }}</x-slot>

    <x-slot name="content">
        <form method="POST" action="{{ route('double-entry.journals.store') }}" id="journal-form" class="space-y-6 rounded-xl bg-white p-6 shadow-sm">
            @csrf
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-3">
                <div>
                    ...
                    <input type="text" name="number" id="number" value="{{ old('number') }}" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                    @error('number')
                ...
                <div>
                    ...
                    <input type="date" name="date" id="date" value="{{ old('date', date('Y-m-d')) }}" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                    @error('date')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="status" id="status" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        <option value="draft">{{ trans('double-entry::general.statuses.draft') }}</option>
                        <option value="posted">{{ trans('double-entry::general.statuses.posted') }}</option>
                    </select>
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="description" id="description" value="{{ old('description') }}" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                </div>

                <div>
                    <label class="mb-1 block text-sm font-medium text-gray-700" for="reference">{{ trans('double-entry::general.reference') }}</label>
                    <input type="text" name="reference" id="reference" value="{{ old('reference') }}" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                </div>
            </div>
                    ...
                    <table class="w-full text-left text-sm" id="lines-table">
                        ...
                        <tbody id="lines-body">
                            <tr class="line-row border-b border-gray-100">
                                <td class="py-2 pr-3">
                                    <select name="lines[0][account_id]" class="w-full rounded-lg border-gray-300 text-sm shadow-sm" required>
                                        <option value="">--</option>
                                ...
                                <td class="py-2 pr-3">
                                    <input type="text" name="lines[0][description]" class="w-full rounded-lg border-gray-300 text-sm shadow-sm">
                                </td>
                                <td class="py-2 pr-3">
                                    <input type="number" name="lines[0][debit]" step="0.0001" min="0" value="0" class="w-full rounded-lg border-gray-300 text-right text-sm shadow-sm debit-input">
                                </td>
                                <td class="py-2 pr-3">
                                    <input type="number" name="lines[0][credit]" step="0.0001" min="0" value="0" class="w-full rounded-lg border-gray-300 text-right text-sm shadow-sm credit-input">
                                </td>
                                <td class="py-2">
                            ...
                            <tr class="line-row border-b border-gray-100">
                                <td class="py-2 pr-3">
                                    <select name="lines[1][account_id]" class="w-full rounded-lg border-gray-300 text-sm shadow-sm" required>
                                        <option value="">--</option>
                                        @foreach($accounts as $account)
                                            <option value="{{ $account->id }}">{{ $account->code }} - {{ $account->name }}</option>
                                        @endforeach
                                ...
                                <td class="py-2 pr-3">
                                    <input type="text" name="lines[1][description]" class="w-full rounded-lg border-gray-300 text-sm shadow-sm">
                                </td>
                                <td class="py-2 pr-3">
                                    <input type="number" name="lines[1][debit]" step="0.0001" min="0" value="0" class="w-full rounded-lg border-gray-300 text-right text-sm shadow-sm debit-input">
                                </td>
                                <td class="py-2 pr-3">
                                    <input type="number" name="lines[1][credit]" step="0.0001" min="0" value="0" class="w-full rounded-lg border-gray-300 text-right text-sm shadow-sm credit-input">
                                </td>
                                <td class="py-2">
                                    <button type="button" onclick="removeLine(this)" class="text-red-400 hover:text-red-600">
                                        <span class="material-icons text-lg">close</span>
