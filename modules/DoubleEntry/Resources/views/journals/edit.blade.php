<x-layouts.admin>
    <x-slot name="title">{{ trans('general.edit') }} {{ $journal->number }}</x-slot>

    <x-slot name="content">
        <x-form.container>
            <x-form id="journal-form" method="PATCH" :route="['double-entry.journals.update', $journal->id]" :model="$journal">
                <x-form.section>
                    <x-slot name="head">
                    ...
                    <x-slot name="body">
                        <x-form.group.date name="date" label="{{ trans('double-entry::general.journal_date') }}" value="{{ $journal->date->format('Y-m-d') }}" />
                        <x-form.group.text name="reference" label="{{ trans('double-entry::general.reference') }}" :value="$journal->reference" not-required />
                        <x-form.group.textarea name="description" label="{{ trans('general.description') }}" :value="$journal->description" not-required />
                        <x-form.group.toggle name="is_recurring" label="Recurring" :value="$journal->is_recurring" />
                        <x-form.group.select name="recurring_frequency" label="Recurring Frequency" :options="['weekly' => 'Weekly', 'monthly' => 'Monthly', 'quarterly' => 'Quarterly']" :selected="$journal->recurring_frequency" not-required />
                        <x-form.group.date name="next_run_at" label="Next Run" :value="optional($journal->next_run_at)->format('Y-m-d')" not-required />
                    </x-slot>
                </x-form.section>
