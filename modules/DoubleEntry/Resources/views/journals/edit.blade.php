<x-layouts.admin>
    ...
    <x-slot name="content">
                ...
                <div class="bg-white rounded-xl shadow-sm p-6 mb-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <input type="date" name="date" value="{{ old('date', $journal->date->format('Y-m-d')) }}" required
                        ...
                        <div>
                            ...
                            <input type="text" name="reference" value="{{ old('reference', $journal->reference) }}" maxlength="100"
                                class="w-full rounded-md border-gray-300 shadow-sm text-sm">
                        </div>
                        ...
                        <div>
                            ...
                            <select name="basis" class="w-full rounded-md border-gray-300 shadow-sm text-sm">
                                <option value="accrual" @selected(old('basis', $journal->basis) === 'accrual')>{{ trans('double-entry::general.accrual') }}</option>
                                <option value="cash" @selected(old('basis', $journal->basis) === 'cash')>{{ trans('double-entry::general.cash') }}</option>
                            </select>
                        </div>

                        <div class="md:col-span-2">
                            <label class="block text-sm font-medium text-gray-700 mb-1">{{ trans('general.description') }}</label>
                            <textarea name="description" rows="2" class="w-full rounded-md border-gray-300 shadow-sm text-sm">{{ old('description', $journal->description) }}</textarea>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="status" class="w-full rounded-md border-gray-300 shadow-sm text-sm">
                                <option value="posted" @selected(old('status', $journal->status) === 'posted')>{{ trans('double-entry::general.statuses.posted') }}</option>
                                <option value="draft" @selected(old('status', $journal->status) === 'draft')>{{ trans('double-entry::general.statuses.draft') }}</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
                        <div>
                            ...
                            <select name="recurring_frequency" class="w-full rounded-md border-gray-300 shadow-sm text-sm">
                                ...
                                <option value="weekly" @selected(old('recurring_frequency', $journal->recurring_frequency) === 'weekly')>{{ trans('double-entry::general.frequencies.weekly') }}</option>
                                <option value="monthly" @selected(old('recurring_frequency', $journal->recurring_frequency) === 'monthly')>{{ trans('double-entry::general.frequencies.monthly') }}</option>
                                <option value="quarterly" @selected(old('recurring_frequency', $journal->recurring_frequency) === 'quarterly')>{{ trans('double-entry::general.frequencies.quarterly') }}</option>
                                <option value="yearly" @selected(old('recurring_frequency', $journal->recurring_frequency) === 'yearly')>{{ trans('double-entry::general.frequencies.yearly') }}</option>
                            </select>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">{{ trans('double-entry::general.next_recurring_date') }}</label>
                            <input type="date" name="next_recurring_date" value="{{ old('next_recurring_date', $journal->next_recurring_date?->format('Y-m-d')) }}"
                                class="w-full rounded-md border-gray-300 shadow-sm text-sm">
                        </div>
                    </div>
                </div>
        ...
        <script>
            ...
            // Initialize with existing lines
            const existingLines = @json($journal->lines);
