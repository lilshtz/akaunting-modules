<x-layouts.admin>
    <x-slot name="title">{{ trans('general.title.edit', ['type' => $account->name]) }}</x-slot>

    <x-slot name="content">
        <form method="POST" action="{{ route('double-entry.accounts.update', $account->id) }}" class="space-y-6 rounded-xl bg-white p-6 shadow-sm">
            @csrf
            @method('PATCH')
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                ...
                <div>
                    ...
                    <input type="text" name="code" id="code" value="{{ old('code', $account->code) }}" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                    @error('code')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $account->name) }}" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                    @error('name')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="parent_id" id="parent_id" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        @foreach($parentAccounts as $id => $name)
                            <option value="{{ $id }}" {{ old('parent_id', $account->parent_id) == $id ? 'selected' : '' }}>{{ $name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="opening_balance" id="opening_balance" value="{{ old('opening_balance', $account->opening_balance) }}" step="0.0001" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                </div>

                <div class="flex items-center gap-2 pt-6">
                    <input type="checkbox" name="enabled" id="enabled" value="1" {{ old('enabled', $account->enabled) ? 'checked' : '' }} class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" id="description" rows="3" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('description', $account->description) }}</textarea>
                </div>
            </div>
